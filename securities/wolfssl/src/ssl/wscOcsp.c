...
static int _initOCSP_Entry(OCSP_Entry* ocspe, DecodedCert* cert)
{
...
static void _freeOCSP_Entry(OCSP_Entry* ocspe)
{
...
static int _xstat2err(int stat)
{
    switch (stat) {
        case CERT_GOOD:
            return 0;
...
int CheckCertOCSP(WOLFSSL_OCSP* ocsp, DecodedCert* cert)
{
    ...
    if (ocspe == NULL) {
        ...
        if (ocspe != NULL) {
            _initOCSP_Entry(ocspe, cert);
            ocspe->next = ocsp->ocspList;
            ocsp->ocspList = ocspe;
        }
        else {
            UnLockMutex(&ocsp->ocspLock);
            WOLFSSL_LEAVE( MEMORY_ERROR);
            return MEMORY_ERROR;
        }
    }
    else {
    ...
    if (certStatus != NULL) {
        if (!ValidateDate(certStatus->thisDate,
            ...
            result = _xstat2err(certStatus->status);
            UnLockMutex(&ocsp->ocspLock);
    ...
    if (result >= 0 && ocspRespBuf) {
        ...
        if (ocspResponse->responseStatus != OCSP_SUCCESSFUL)
            ...
            if (CompareOcspReqResp(ocspRequest, ocspResponse) == 0) {
                result = _xstat2err(ocspResponse->status->status);
