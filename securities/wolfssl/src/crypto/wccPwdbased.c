...
#ifndef NO_SHA
/* PBKDF1 needs at least SHA available : PKCS5 
...
static int _getDigestSize(int hashType)
{
	int hLen;
...
/* PKCS 5 v2.0 : calculate hash based on passwd, salt with algorithm of HMAC */
...
static int _getPKCS12HashSizes(int hashType, word32* v, word32* u)
{
    if (!v || !u)
        return BAD_FUNC_ARG;
...
/* helper for PKCS12_PBKDF(), does hash operation: MD5/SHAxxx */
static int _doPKCS12Hash(int hashType, byte* buffer, word32 totalLen,
...
/* PKCS 12 : calculate hash based on passwd and salt */
                 ...
                 int saltLen, int iterations, int kLen, int hashType, int id)
{
    ...
    ret = _getPKCS12HashSizes(hashType, &v, &u);
    if (ret < 0)
        return BAD_FUNC_ARG;
    ...
    while (kLen > 0) {
        ...
        ret = _doPKCS12Hash(hashType, buffer, totalLen, Ai, u, iterations);
        if (ret < 0)
            break;
