...
#ifndef __WCK_ECC_H__
#define __WCK_ECC_H__
...
typedef struct {
	...
	int idx;            /* Index into the ecc_sets[] for the parameters of
	                   ...
	                   curve in dp */
	const ecc_set_type* dp;     /* domain parameters, either points to NIST curves (idx >= 0) or user supplied */
...
WOLFSSL_API int wc_ecc_make_key(RNG* rng, int keysize, ecc_key* key);
...
WOLFSSL_API int wc_ecc_sign_hash(const byte* in, word32 inlen, byte* out, word32 *outlen, RNG* rng, ecc_key* key);
...
WOLFSSL_API void wc_ecc_free(ecc_key* key);
...
WOLFSSL_API ecEncCtx* wc_ecc_ctx_new(int flags, RNG* rng);
WOLFSSL_API void wc_ecc_ctx_free(ecEncCtx*);
WOLFSSL_API int wc_ecc_ctx_reset(ecEncCtx*, RNG*);   /* reset for use again w/o alloc/free */
...
WOLFSSL_API int wc_ecc_encrypt(ecc_key* privKey, ecc_key* pubKey, const byte* msg,
                word32 msgSz, byte* out, word32* outSz, ecEncCtx* ctx);
WOLFSSL_API int wc_ecc_decrypt(ecc_key* privKey, ecc_key* pubKey, const byte* msg,
                word32 msgSz, byte* out, word32* outSz, ecEncCtx* ctx);
