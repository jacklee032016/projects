...
#ifndef __WSI_INTERNEL_H__
#define __WSI_INTERNEL_H__
...
typedef	struct WOLFSSL_BIO_METHOD{
...
/* wolfSSL BIO type */
typedef	struct WOLFSSL_BIO{
	byte        type;          /* method type */
	byte        close;         /* close flag */
	byte        eof;           /* eof flag */
	struct	WOLFSSL*     ssl;           /* possible associated ssl */
	byte*       mem;           /* memory buffer */
	int         memLen;        /* memory buffer length */
	int         fd;            /* possible file descriptor */
	struct WOLFSSL_BIO* prev;          /* previous in chain */
	struct WOLFSSL_BIO* next;          /* next in chain */
}WOLFSSL_BIO;
...
typedef	struct WOLFSSL {
    ...
    struct WOLFSSL_ALERT_HISTORY alert_history;
    int             error;
...
static const byte client[SIZEOF_SENDER] = { 0x43, 0x4C, 0x4E, 0x54 };	/* C L N T */
static const byte server[SIZEOF_SENDER] = { 0x53, 0x52, 0x56, 0x52 };	/* S R V R */
