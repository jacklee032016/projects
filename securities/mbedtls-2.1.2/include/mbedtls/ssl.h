...
/**
 *                Return the name of the ciphersuite associated with the
 *                      given ID
 *
...
/**
 *                Return the ID of the ciphersuite associated with the
 *                      given name
 *
...
/**
 *           Set the underlying BIO callbacks for write, read and
 *                 read-with-timeout.
 *
 * \param ssl      SSL context
...
/**
 *           Set the timeout period for mbedtls_ssl_read()
 *                 (Default: no timeout.)
 *
...
#if defined(MBEDTLS_SSL_DTLS_BADMAC_LIMIT)
/**
 *           Set a limit on the number of records with a bad MAC
...
/**
 *                Set the list of allowed ciphersuites and the preference
...
/**
 *                Set the list of allowed ciphersuites and the
...
/**
 *           Set the data required to verify peer certificate for the
 *                 current handshake
 *
...
/**
 *           Set server side ServerName TLS extension callback
...
#if defined(MBEDTLS_SSL_ENCRYPT_THEN_MAC)
/**
 *            Enable or disable Encrypt-then-MAC
...
#if defined(MBEDTLS_ARC4_C)
/**
 *           Disable or enable support for RC4
...
#if defined(MBEDTLS_SSL_TRUNCATED_HMAC)
/**
 *           Activate negotiation of truncated HMAC
...
#if defined(MBEDTLS_SSL_CBC_RECORD_SPLITTING)
/**
 *           Enable / Disable 1/n-1 record splitting
...
#if defined(MBEDTLS_SSL_RENEGOTIATION)
/**
 *           Enable / Disable renegotiation support for connection when
 *                 initiated by peer
...
/**
 *           Return the (maximum) number of bytes added by the record
...
/**
 *           Free referenced items in an SSL session including the
