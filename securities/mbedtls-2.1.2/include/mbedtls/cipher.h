...
typedef enum {
    MBEDTLS_MODE_NONE = 0,
    MBEDTLS_MODE_ECB,		/* Electronic Code Book */
    MBEDTLS_MODE_CBC,		/* Cipher Block Chaining */
    MBEDTLS_MODE_CFB,		/* Cipher FeedBack */
    MBEDTLS_MODE_OFB, /* Unused! */
    MBEDTLS_MODE_CTR,		/* Counter mode */
    MBEDTLS_MODE_GCM,		/* Galois Counter Mode */
    MBEDTLS_MODE_STREAM,
    MBEDTLS_MODE_CCM,		/* Counter with CBC-MAC */
 ...
 */
typedef struct _mbedtls_cipher_base_t mbedtls_cipher_base_t;
 ...
 *     initialised using cipher_context_from_type or  cipher_context_from_string.
 */
 ...
 * \param iv_len :  IV length for ciphers with variable-size IV; discarded by ciphers with fixed-size IV.
...
/**
 *  Generic cipher update function. Encrypts/decrypts using the given cipher context. Writes as many block
 *  size'd blocks of data as possible to output. Any data that cannot be written immediately will either be added
 *  to the next block, or flushed when cipher_final is called.
 *   Exception: for MBEDTLS_MODE_ECB, expects single block in size (e.g. 16 bytes for AES)
 * \param input         buffer holding the input data
 * \param ilen          length of the input data
 * \param output        buffer for the output data. Should be able to hold at
 *                      least ilen + block_size. Cannot be the same buffer as input!
 * \param olen     length of the output data, will be filled with the actual number of bytes written.
 *
...
/**
 *  Generic cipher finalisation function. If data still needs to be flushed from an incomplete block, data
 *  contained within it will be padded with the size of the last block, and written to the output buffer.
...
#if defined(MBEDTLS_GCM_C)
/**
 *  Write tag for AEAD ciphers. Currently only supported with GCM.
