...
{
	struct serverstruct *tp = (struct serverstruct *)calloc(1, sizeof(struct serverstruct));
	tp->next = servers;
	tp->sd = sd;
	servers = tp;
}
...
static void handlenewconnection(int listenfd, int is_ssl) 
{
	...
	if (tp == sizeof(struct sockaddr_in6)) 
		...
		inet_ntop(AF_INET, &(((struct sockaddr_in *)&their_addr)->sin_addr), ipbuf, sizeof(ipbuf));
	else 
		*ipbuf = '\0';
...
static int openlistener(char *address, int port) 
{
	...
	my_addr.sin_addr.s_addr = address == NULL ? INADDR_ANY : inet_addr(address);
#else
...
//	|SSL_DISPLAY_ALL
	servers->ssl_ctx = ssl_ctx_new(options|SSL_DISPLAY_BYTES, CONFIG_HTTP_SESSION_CACHE_SIZE);
	servers->is_ssl = 1;
	...
	while (1)
	{
		...
		active = select(wnum > rnum ? wnum+1 : rnum+1, rnum != -1 ? &rfds : NULL, wnum != -1 ? &wfds : NULL,
