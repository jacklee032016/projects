...
static void buildactualfile(struct connstruct *cn)
{
	...
	if (isdir(cn->actualfile) && cn->actualfile[strlen(cn->actualfile)-1] != '/')
...
static int special_write(struct connstruct *cn, const char *buf, size_t count)
{
	if (cn->is_ssl)
	{
		SSL *ssl = cn->ssl;
...
void procDoDir(struct connstruct *cn) 
{
	...
	do 
	{
		...
		if (cn->filereq[0] == '/' && cn->filereq[1] == '\0' && strcmp(file, "..") == 0) 
			continue;
...
static void proccgi(struct connstruct *cn) 
{
	...
	snprintf(cgienv[0], MAXREQUESTLENGTH, HTTP_VERSION" 200 OK\nServer: %s\n%s",
	...
	*/
	...
	snprintf(cgienv[cgi_index++], MAXREQUESTLENGTH,	"SERVER_NAME=%s", cn->server_name);
	sprintf(cgienv[cgi_index++], "SERVER_PORT=%d", cn->is_ssl ? CONFIG_HTTP_HTTPS_PORT : CONFIG_HTTP_PORT);
	snprintf(cgienv[cgi_index++], MAXREQUESTLENGTH,	"REQUEST_URI=%s", cn->uri_request);
	snprintf(cgienv[cgi_index++], MAXREQUESTLENGTH,	"SCRIPT_NAME=%s", cn->filereq);
	snprintf(cgienv[cgi_index++], MAXREQUESTLENGTH,	"PATH_INFO=%s", cn->uri_path_info);
	snprintf(cgienv[cgi_index++], MAXREQUESTLENGTH,	"QUERY_STRING=%s", cn->uri_query);
	snprintf(cgienv[cgi_index++], MAXREQUESTLENGTH,	"REMOTE_ADDR=%s", cn->remote_addr);
	snprintf(cgienv[cgi_index++], MAXREQUESTLENGTH,	"HTTP_COOKIE=%s", cn->cookie);  /* note: small size */
#if defined(CONFIG_HTTP_HAS_AUTHORIZATION)
	snprintf(cgienv[cgi_index++], MAXREQUESTLENGTH,	"REMOTE_USER=%s", cn->authorization);
#endif
...
static int check_digest(char *salt, const char *msg_passwd)
{
	...
	uint8_t real_passwd[HASH_MD_LENGTH_MD5];
	int salt_size = sizeof(b256_salt);
	int password_size = sizeof(real_passwd);
	char *b64_passwd;
	uint8_t md5_result[HASH_MD_LENGTH_MD5];
	MD5_CTX ctx;
	...
	*b64_passwd++ = 0;
	...
	if (base64_decode(b64_passwd, strlen(b64_passwd), real_passwd, &password_size))
		return -1;
	...
	return memcmp(md5_result, real_passwd, HASH_MD_LENGTH_MD5);/* 0 = ok */
}
...
#ifdef CONFIG_HTTP_HAS_AUTHORIZATION
	else if (strcasecmp(buf, "Authorization:") == 0 && strncmp(value, "Basic ", 6) == 0)
	{
		int size = sizeof(cn->authorization);
		if (base64_decode(&value[6], strlen(&value[6]), (uint8_t *)cn->authorization, &size))
