...
{
    return send_packet(ssl, TLS_CNT_HANDSHAKE, g_cert_request, sizeof(g_cert_request));
}
...
static int process_cert_verify(SSL *ssl)
{
    ...
    n = RSA_decryptOrVerify(x509_ctx->rsa_ctx, &buf[6], dgst_buf, sizeof(dgst_buf), 0);
    SSL_CTX_UNLOCK(ssl->ctx->mutex);

    if (n != HASH_MD_LENGTH_SHA1 + HASH_MD_LENGTH_MD5)
    {
    ...
    if (memcmp(dgst_buf, dgst, HASH_MD_LENGTH_MD5 + HASH_MD_LENGTH_SHA1))
    {
        ret = SSL_ERROR_INVALID_KEY;
    }
...
static int send_server_hello(SSL *ssl)
{
	...
	/* server random value */
	if (get_random(TLS_RANDOM_SIZE, &buf[6]) < 0)
		return SSL_NOT_OK;
	
	header->hsType = HS_SERVER_HELLO;
	hello->major = 0x03;
...
#endif
	{
		...
		{
			memcpy(ssl->session->session_id, ssl->session_id, SSL_SESSION_ID_SIZE);
		}
...
{
	return send_packet(ssl, TLS_CNT_HANDSHAKE, g_hello_done, sizeof(g_hello_done));
}
...
static int process_client_hello(SSL *ssl)
{
    uint8_t *buf = ssl->bm_data;
    int pkt_size = ssl->bm_index;
    int i, j, cs_len, id_len, offset = 6 + TLS_RANDOM_SIZE;
    int ret = SSL_OK;
    ...
    memcpy(ssl->dc->client_random, &buf[6], TLS_RANDOM_SIZE);
...
#ifndef CONFIG_SSL_SKELETON_MODE
    ssl->session = ssl_session_update(ssl->ctx->num_sessions, ssl->ctx->ssl_sessions, ssl, id_len ? &buf[offset] : NULL);
#endif
...
static int process_client_key_xchg(SSL *ssl)
{
    ...
    /* rsa_ctx->bi_ctx is not thread-safe */
    SSL_CTX_LOCK(ssl->ctx->mutex);
    premaster_size = RSA_decryptOrVerify(rsa_ctx, &buf[offset], premaster_secret,
            sizeof(premaster_secret), 1);
    SSL_CTX_UNLOCK(ssl->ctx->mutex);

    if (premaster_size != SSL_MASTER_SECRET_SIZE || 
                ...
                premaster_secret[1] != (ssl->client_version & 0x0f))
    {
        ...
        if (get_random(SSL_MASTER_SECRET_SIZE, premaster_secret) < 0)
            return SSL_NOT_OK;
...
#if 0
	print_blob(premaster_secret, SSL_MASTER_SECRET_SIZE, "pre-master" );
#endif
 ...
 */
EXP_FUNC SSL * STDCALL ssl_server_new(SSL_CTX *ctx, int client_fd)
{
	SSL *ssl;

	ssl = ssl_new(ctx, client_fd);
	ssl->next_state = HS_CLIENT_HELLO;

#ifdef CONFIG_SSL_FULL_MODE
	if (ctx->chain_length == 0)
		AX_LOG("Warning - no server certificate defined\n"); TTY_FLUSH();
#endif
...
int do_svr_handshake(SSL *ssl, HAND_SHAKE_T handshake_type, uint8_t *buf, int hs_len)
{
    ...
    switch (handshake_type)
    {
        ...
        case HS_CERTIFICATE:/* the client sends its cert */
            ...
            { 
                int cert_res;
                cert_res = x509_verify( ssl->ctx->ca_cert_ctx, ssl->x509_ctx);
...
int process_sslv23_client_hello(SSL *ssl)
{
    ...
    DISPLAY_BYTES(ssl, buf, read_len, "received %d bytes", read_len);
