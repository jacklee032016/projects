                        ...
                        uint8_t *buf, int *offset)
{
    ...
    uint8_t *block = (uint8_t *)alloca(sizeof(asn1_sig) + HASH_MD_LENGTH_SHA1);
    int sig_size;

    /* add the digest as an embedded asn.1 sequence */
    memcpy(block, asn1_sig, sizeof(asn1_sig));
    memcpy(&block[sizeof(asn1_sig)], sha_dgst, HASH_MD_LENGTH_SHA1);

    sig_size = RSA_encryptOrSign(rsa_ctx, block, 
                            sizeof(asn1_sig) + HASH_MD_LENGTH_SHA1, enc_block, 1);
...
EXP_FUNC int STDCALL ssl_x509_create(SSL_CTX *ssl_ctx, uint32_t options, const char * dn[], uint8_t **cert_data)
{
    ...
    int seq_size = pre_adjust_with_size(ASN1_SEQUENCE,  &seq_offset, buf, &offset);
