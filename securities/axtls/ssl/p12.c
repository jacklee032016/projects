 ...
 *
 * The decoding of a PKCS#12 key is fairly specific - this code was tested on a key generated with:
 *
                        ...
                        uint8_t *priv_key, int priv_key_len, int id)
{
    ...
    uint8_t Ai[HASH_MD_LENGTH_SHA1];
    SHA1_CTX sha_ctx;
    RC4_CTX rc4_ctx;
    int i;
    ...
    {
        SHA1_Init(&sha_ctx);
        SHA1_Update(&sha_ctx, Ai, HASH_MD_LENGTH_SHA1);
        SHA1_Final(Ai, &sha_ctx);
    }
    ...
    if (id == PKCS12_KEY_ID)
    {
        ...
        memcpy(priv_key, Ai, HASH_MD_LENGTH_SHA1);
...
int pkcs8_decode(SSL_CTX *ssl_ctx, SSLObjLoader *ssl_obj, const char *password)
{
    ...
    if (asn1_next_obj(buf, &offset, ASN1_SEQUENCE) < 0)
    {
        ...
        AX_LOG("Error: Invalid p8 ASN.1 file\n");
#endif
        goto error;
    }
        ...
        const uint8_t **salt, int *iterations)
{
    ...
    {
#ifdef CONFIG_SSL_FULL_MODE
        AX_LOG("Error: pkcs8/pkcs12 must use \"PBE-SHA1-RC4-128\"\n");
#endif
        goto error;
    }
...
int pkcs12_decode(SSL_CTX *ssl_ctx, SSLObjLoader *ssl_obj, const char *password)
{
    ...
    uint8_t mac[HASH_MD_LENGTH_SHA1];
    const uint8_t *salt;
    ...
    {
#ifdef CONFIG_SSL_FULL_MODE
        AX_LOG("Error: Invalid p12 ASN.1 file\n");
#endif
        goto error;
    }
    ...
    if ((ret = p8_decrypt(uni_pass, uni_pass_len, salt, iterations, 
                            key, HASH_MD_LENGTH_SHA1, PKCS12_MAC_ID)) < 0)
        goto error;

    hmac_sha1(auth_safes, auth_safes_len, key, HASH_MD_LENGTH_SHA1, mac);

    if (memcmp(mac, orig_mac, HASH_MD_LENGTH_SHA1))
    {
