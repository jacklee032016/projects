...
static const uint8_t PADDING[HASH_DATA_LENGTH_MD5] = 
{
...
{
    uint32_t a = state[0], b = state[1], c = state[2], d = state[3], x[HASH_MD_LENGTH_MD5];
...
EXP_FUNC void STDCALL MD5_Final(uint8_t *digest, MD5_CTX *ctx)
{
	...
	Encode(digest, ctx->state, HASH_MD_LENGTH_MD5);
}
