/**
 * Implements the RSA public encryption algorithm. Uses the bigint library to perform its calculations.
 */

#include "crypto.h"

/* init all members in RSA, include public key(modulus(Phi), exponention) and private key d*/
EXP_FUNC void STDCALL RSA_priv_key_new(RSA_CTX **ctx, 
...
/* modulus is Phi(¦Õ), exp is exponention, which gcd is 1 with Phi */
EXP_FUNC void STDCALL RSA_pub_key_new(RSA_CTX **ctx, const uint8_t *modulus, int mod_len, const uint8_t *pub_exp, int pub_len)
{
	RSA_CTX *rsa_ctx;
	BI_CTX *bi_ctx;
...
EXP_FUNC void STDCALL RSA_free(RSA_CTX *rsa_ctx)
{
	BI_CTX *bi_ctx;
...
/* Performs m = c^d mod n: decryption */
...
/* Performs c = m^e mod n: bi_msg is the base of exponention. e is RSA public key 
*/
 ...
 */
EXP_FUNC int STDCALL RSA_encryptOrSign(const RSA_CTX *ctx, const uint8_t *in_data, uint16_t in_len, 
 ...
 * @see http://www.rsasecurity.com/rsalabs/node.asp?id=2125
 */
EXP_FUNC int STDCALL RSA_decryptOrVerify(const RSA_CTX *ctx, const uint8_t *in_data, 
                            uint8_t *out_data, int out_len, int is_decryption)
{
	...
	{
		AX_LOG("output buffer is so small: %d<%d bytes\n", out_len, byte_size);
		return -1;
	}
...
EXP_FUNC void STDCALL RSA_print(const RSA_CTX *rsa_ctx) 
{
	if (rsa_ctx == NULL)
	return;
