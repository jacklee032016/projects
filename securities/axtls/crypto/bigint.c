...
static void check(const bigint *bi)
{
	if (bi->refs <= 0)
	{
		AX_LOG("check: zero or negative refs in bigint\n");
		abort();
	}

	if (bi->next != NULL)
	{
		AX_LOG("check: attempt to use a bigint from the free list\n");
		abort();
	}
}
#else
...
/* Make a new empty bigint. It may just use an old one if one is available. Otherwise get one off the heap */
static bigint *alloc(BI_CTX *ctx, int size)
{
	...
	else/* not associated with the ctx, except the active_count???, lzj */
...
EXP_FUNC void STDCALL bi_terminate(BI_CTX *ctx)
{
	...
	{
#ifdef CONFIG_SSL_FULL_MODE
		AX_LOG("bi_terminate: there were %d un-freed bigints\n",ctx->active_count);
#endif
		abort();
	}
...
void bi_permanent(bigint *bi)
{
	...
	{
#ifdef CONFIG_SSL_FULL_MODE
		AX_LOG("bi_permanent: refs was not 1\n");
#endif
		abort();
	}
...
void bi_depermanent(bigint *bi)
{
	...
	{
#ifdef CONFIG_SSL_FULL_MODE
		AX_LOG("bi_depermanent: bigint was not permanent\n");
#endif
		abort();
	}
...
EXP_FUNC void STDCALL bi_free(BI_CTX *ctx, bigint *bi)
{
	...
	{
#ifdef CONFIG_SSL_FULL_MODE
		AX_LOG("bi_free: active_count went negative - double-freed bigint?\n");
#endif
		abort();
	}
}
 ...
 */
EXP_FUNC bigint * STDCALL bi_import(BI_CTX *ctx, const uint8_t *data, int size)
{
...
EXP_FUNC bigint* STDCALL bi_str_import(BI_CTX *ctx, const char *data)
{
	...
	for (i = size-1; i >= 0; i--)
	{
/*		the hex number encoded in string format must be in upper case.
		Should not confusion with the console output of printf which always output hex number in low case characters
		int num = (data[i] <= '9') ? (data[i] - '0') : (data[i] - 'A' + 10);
*/		
		int num = CHAR_2_NUM(data[i]);
...
EXP_FUNC void STDCALL bi_print(const char *label, bigint *x)
{
	int i, j;
 ...
 * zeroed.
 */
EXP_FUNC void STDCALL bi_export(BI_CTX *ctx, bigint *x, uint8_t *data, int size)
{
	int i, j, k = size-1;
 ...
 * @return The result of the multiplication.
 */
EXP_FUNC bigint* STDCALL bi_square(BI_CTX *ctx, bigint *bia)
{
    check(bia);
 ...
 * @param ctx [in]  The bigint session context.
 * bi  [in]  The bigint on which to perform the mod power operation. In "b^e mod m", it is "b"
 * biexp [in] The bigint exponent, this is "e" in the last formula
 * The result of the mod exponentiation operation
 * @see bi_set_mod().
 */
bigint* STDCALL bi_mod_power(BI_CTX *ctx, bigint *bi, bigint *biexp)
{
