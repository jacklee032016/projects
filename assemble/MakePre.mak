...
cflags = $(cflags) /Iinclude /Icrypto /Issl
...
{$(SRC_DIR)}.s{$(OBJDIR)}.o:
    $(YASM) -f win32  -p gas -l $*.list -o $@  $**

{$(SRC_DIR)}.asm{$(OBJDIR)}.obj:
    $(YASM) -f win32  -p nasm -l $*.list -o $@  $**



#-g cv8 -f win%sz% asm\x%arch%\%asm%.asm -o obj\x%arch%\%asm%.obj -p gas 
...
LIB_HW=$(OUTDIR)\libHwUtils
LIB_X509=$(OUTDIR)\libAx509
LIB_TLS=$(OUTDIR)\libAxTls
